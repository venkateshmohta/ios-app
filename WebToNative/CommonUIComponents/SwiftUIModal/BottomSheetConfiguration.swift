...
//  WebToNative
//
//  Created by Akash Kamati on 11/06/24.
...
public struct BottomSheetConfiguration {
    ...
    public var background:AnyView = AnyView(RoundedRectangle(cornerRadius: 32).fill(Color.white))
    /// A view that is used as the dim behind the bottom sheet content.
    public var dim:AnyView = AnyView(Color.black.opacity(0.3))
    /// A view that is used as a drag indicator on top of the bottom sheet content.
    public var indicator:AnyView = AnyView(
